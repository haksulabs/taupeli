...
model = load_model(modelsavepath + 'model_dx_cond.joblib')
...
for s in stags:
    for c in conds:        
        ...
        x_values = np.linspace(df['delta_xf_end'].min(), df['delta_xf_end'].max(), 100)
#        pre_df = pd.DataFrame({'abs_ttcdiff': x_values, 'stag1':s, 'd_updown':c['d_updown'] , 'd_leftright':c['d_leftright'], 'd_diagonal': c['d_diagonal'] })
        pre_df = pd.DataFrame({'delta_xf_end': x_values, 'stag1':s, 'd_updown':c['d_updown'] , 'd_leftright':c['d_leftright'], 'd_diagonal': c['d_diagonal'] })

        predict_df = pd.concat([predict_df,pre_df], ignore_index=True)
                        
# interactions

predict_df['delta_xf_end:d_updown'] = predict_df['delta_xf_end'] *  predict_df['d_updown']
predict_df['delta_xf_end:d_leftright'] = predict_df['delta_xf_end'] *  predict_df['d_leftright']
predict_df['delta_xf_end:d_diagonal'] = predict_df['delta_xf_end'] *  predict_df['d_diagonal']
...
updown_data_stag = predict_df[  (predict_df['d_updown'] == 1) & (predict_df['stag1']==-1)]
updown_data_stag1 = predict_df[  (predict_df['d_updown'] == 1) & (predict_df['stag1']==1)]

leftright_data = predict_df[  (predict_df['d_leftright'] == 1) & (predict_df['stag1']==0)]
leftright_data_stag = predict_df[  (predict_df['d_leftright'] == 1) & (predict_df['stag1']==-1)]
leftright_data_stag1 = predict_df[  (predict_df['d_leftright'] == 1) & (predict_df['stag1']==1)]


diagonal_data = predict_df[  (predict_df['d_diagonal'] == 1) & (predict_df['stag1']==0)]
diagonal_data_stag = predict_df[  (predict_df['d_diagonal'] == 1) & (predict_df['stag1']==-1)]
diagonal_data_stag1 = predict_df[  (predict_df['d_diagonal'] == 1) & (predict_df['stag1']==1)]


 
# Plotting
plt.figure(figsize=(10, 6))
plt.plot(updown_data['delta_xf_end'], updown_data['predicted_accuracy'], label='Up/Down')
plt.plot(leftright_data['delta_xf_end'], leftright_data['predicted_accuracy'], linestyle='--', label='Left/Right')
plt.plot(diagonal_data['delta_xf_end'], diagonal_data['predicted_accuracy'], linestyle=':', label='Diagonal')
 
plt.plot(updown_data_stag['delta_xf_end'], updown_data_stag['predicted_accuracy'], label='Up/Down  stag -1')
plt.plot(leftright_data_stag['delta_xf_end'], leftright_data_stag['predicted_accuracy'], linestyle='--', label='Left/Right stag -1')
plt.plot(diagonal_data_stag1['delta_xf_end'], diagonal_data_stag['predicted_accuracy'], linestyle=':', label='Diagonal stag -1')

plt.plot(updown_data_stag1['delta_xf_end'], updown_data_stag1['predicted_accuracy'], label='Up/Down')
plt.plot(leftright_data_stag1['delta_xf_end'], leftright_data_stag1['predicted_accuracy'], linestyle='--', label='Left/Right stag +1')
plt.plot(diagonal_data_stag1['delta_xf_end'], diagonal_data_stag1['predicted_accuracy'], linestyle=':', label='Diagonal stag +1')
